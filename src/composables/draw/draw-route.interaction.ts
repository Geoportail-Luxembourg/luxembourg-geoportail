...
export class DrawRouteInteraction extends PointerInteraction {
  ...
  private adoptExistingSketch_(existingSketch: Feature): void {
    ...
    if (this.sketchCoords_.length >= 2) {
      this.finishCoordinate_ = this.sketchCoords_[this.sketchCoords_.length - 2].slice()
      ...
      if (data.success) {
        ...
        if (this.finishAfterRoute_) {
          ...
          if (curCoordinates.length >= 2) {
            this.finishCoordinate_ = curCoordinates[curCoordinates.length - 2].slice()
          }
        }
      }
    } catch (error) {
 ...
 */
function handleEvent_(this: DrawRouteInteraction, event: MapBrowserEvent<any>): boolean {
 ...
 * Handle down event
 */
function handleDownEvent_(this: DrawRouteInteraction, event: MapBrowserEvent<any>): boolean {
  return true
}
 ...
 * Handle up event  
 */
function handleUpEvent_(this: DrawRouteInteraction, event: MapBrowserEvent<any>): boolean {
 ...
 * Handle move event
 */
function handleMoveEvent_(this: DrawRouteInteraction, event: MapBrowserEvent<any>): boolean {
