...
import { OffLineLayer, OffLineLayerOptions } from '@/composables/offline/offline.model'
...
export abstract class OlLayerTileOfflineHelper {
  ...
  createTileLoadFunction(): LoadFunction {
    ...
    const loadFunction = (tile: ImageTile, src: string) => {
      ...
      localforage
        ...
        .then((content: string) => {
          (<HTMLImageElement>tile.getImage()).src = 
            !content ?
  ...
  abstract deserializeTileLayer(options: OffLineLayerOptions): Options<TileSource>
