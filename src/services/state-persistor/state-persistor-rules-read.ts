 ...
 *
 * Legacy in static-ngeo/js/StateManager.js l. 85
 */
class StatePersistorRulesRead implements StatePersistorRules {
  processRules(paramKeys: ParamKeys) {
  ...
  processRulesForKey(key: string, paramKeys: ParamKeys) {
  ...
  ruleUseLocalStorage(paramKeys: ParamKeys) {
    return [ruleEmptyParams, ruleOneParamOnly, rule2].reduce(
...
const rule2 = (paramKeys: ParamKeys) => {
  // TODO: rename
  return (
...
const statePersistorRulesRead = new StatePersistorRulesRead()
export default statePersistorRulesRead
