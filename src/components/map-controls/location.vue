...
onMounted(() => useControl(Control, {...props, ...{ target: controlElement }}))
</script>

<template>
    <div ref="controlElement" :class="`tracker-off ${props.className} ${CLASS_UNSELECTABLE} ${CLASS_CONTROL}`">
        <button :title="t(props.tipLabel)" @click="handleCenterToLocation">{{ props.label }}</button>
    </div>
</template>