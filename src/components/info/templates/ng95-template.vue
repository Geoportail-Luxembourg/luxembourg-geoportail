...
<template>
  <InfoFeatureLayout :layers="layers" :currentUrl="currentUrl">
    <template #feature-content="{ feature }">
      ...
      <span>{{ t('Elevation') }}</span> : {{ feature.attributes['nouv_cote']
      }}<br />
      <span>{{ t('Type') }}</span> : {{ feature.attributes['type_rivet']
      }}<br />
      <span>{{ t('Croquis') }}</span> :
      <span v-if="feature.attributes.has_sketch">
        <a
          ...
          target="_blank"
          download
          :href="`${DOWNLOAD_SKETCH_URL}?name=${feature.attributes.nom_croq}`"
        >
