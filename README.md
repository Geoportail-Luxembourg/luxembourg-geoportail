...
Because v4 is a standalone app with no backend, it uses sometimes local v3's backend and sometimes migration platform https://migration.geoportail.lu (EDIT: warning, migration is down for now, using: https://map4gim.geoportail.lu instead) to perform api calls (see dedicated `.env` files to check urls).
